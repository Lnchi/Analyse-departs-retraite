...
d3.dsv(";", "departretraite_parcsp.csv").then(data => {
    ...
    const categories = Array.from(new Set(data.map(d => d["Catégorie socioprofessionnelle"])));
    ...
    // =============================
    const svg1 = d3.select("#graph-global")
        .append("svg")
    ...
    const selectedColorGlobal = "#FF0000"; // rouge fluo
    ...
    svg1.selectAll("rect")
        ...
        .on("click", function(event, d){
            ...
            d3.select("#yearSlider").property("value", d.annee);
        });
    ...
    const svgDonutGlobal = d3.select("#donut-global")
        .append("svg")
    ...
    function updateDonutGlobal(selectedYear){
        const arcs = svgDonutGlobal.selectAll("path")
        ...
        arcs.enter()
            ...
            .transition()
            .duration(600)
            .attr("fill", d => d.data.annee===selectedYear ? selectedColorGlobal : "steelblue")
    ...
    // =============================
    const svg2 = d3.select("#graph-csp")
        .append("svg")
        .attr("width", width + margin.left + margin.right)
        .attr("height", height + margin.top + margin.bottom)
        .append("g")
    ...
    svg2.selectAll(".yearGroup")
        ...
        .on("click", function(event,d){
            if(selectedBarCSP) selectedBarCSP.attr("fill", color(selectedBarCSP.datum()["Catégorie socioprofessionnelle"]));
            d3.select(this).attr("fill", selectedColorGlobal);
