...
d3.dsv(";", "departretraite_parcsp.csv").then((data) => {
    ...
    data.forEach(d => {
        ...
        d.sante_limitee = parseFr(d["Proportion de personnes fortement limitées au cours de la première année de retraite (%)"]);
...
const svg = d3.select("#graph-scatter-sante")
    .html("")
    .append("svg")
    .attr("width", "100%")
...
chart.append("rect")
    ...
    .on("click", () => {
        ...
        chart.selectAll("circle.dot").transition().duration(200)
            .attr("opacity", 0.6).attr("stroke", "#fff").attr("stroke-width", 1)
...
// Échelles
const x = d3.scaleLinear().domain([0, d3.max(data, d => d.sante_limitee)]).nice().range([0, width]);
const y = d3.scaleLinear().domain([0, d3.max(data, d => d.duree_sans_emploi)]).nice().range([height, 0]);
const r = d3.scaleSqrt().domain(d3.extent(data, d => d.duree_emploi)).range([5, 22]);
...
chart.append("g")
    ...
    .attr("fill", "#2d3a4b").style("text-anchor", "middle").style("font-weight", "bold")
...
chart.append("line").attr("x1", x(meanSante)).attr("y1", 0).attr("x2", x(meanSante)).attr("y2", height)
    .attr("stroke", "#ccc").attr("stroke-dasharray", "4");

chart.append("line").attr("x1", 0).attr("y1", y(meanSansEmp)).attr("x2", width).attr("y2", y(meanSansEmp))
    .attr("stroke", "#ccc").attr("stroke-dasharray", "4");

// Menu déroulant
const selectDiv = d3.select("#controls-sante").html("");
selectDiv.append("label").text("Filtrer par année : ").style("margin-right", "10px");

const select = selectDiv.append("select")
    .style("padding", "5px 15px").style("border-radius", "20px")
    .on("change", function() { updateScatter(this.value); });
...
function updateScatter(selectedYear) {
    ...
    const circles = chart.selectAll("circle.dot")
    ...
    circles.join(
        enter => enter.append("circle")
            ...
            .on("click", function(event, d) {
                ...
                const infoGroup = chart.append("g")
                    .attr("class", "info-group")
                    .attr("transform", `translate(${infoX},${infoY})`)
                    .style("pointer-events", "none");

                infoGroup.append("rect")
                    .attr("width", 220).attr("height", boxHeight)
                ...
                text.append("tspan").attr("x", 10).style("font-weight", "bold").text(line1);
                if (line2) text.append("tspan").attr("x", 10).attr("dy", "1.2em").style("font-weight", "bold").text(line2);
                text.append("tspan").attr("x", 10).attr("dy", "1.6em").text(`• Santé limitée : ${d.sante_limitee}%`);
                text.append("tspan").attr("x", 10).attr("dy", "1.2em").text(`• Sans emploi : ${d.duree_sans_emploi} ans`);
                text.append("tspan").attr("x", 10).attr("dy", "1.2em").text(`• Durée emploi : ${d.duree_emploi} ans`);
            })
...
// Légende interactive
const legend = chart.append("g").attr("transform", `translate(${width + 35},10)`);

categories.forEach((cat, i) => {
    const row = legend.append("g").attr("transform", `translate(0,${i*38})`).style("cursor","pointer");

    row.append("circle").attr("r", 6).attr("fill", color(cat));

    const [l1, l2] = splitText(cat, 25);
    const textEl = row.append("text").attr("x", 18).attr("y", 4)
    ...
    row.on("click", function(event) {
        ...
        chart.selectAll("circle.dot")
            ...
            .attr("opacity", d => !selectedCategory ? 0.6 : (d.csp_clean === selectedCategory ? 0.9 : 0.1))
            .attr("r", d => !selectedCategory ? r(d.duree_emploi) : (d.csp_clean === selectedCategory ? r(d.duree_emploi) : r(d.duree_emploi) * 0.6));
    ...
    // =============================================================
    const marginHM = { top: 30, right: 40, bottom: 40, left: 180 };
    const widthHM = 850 - marginHM.left - marginHM.right;
    const heightHM = 400 - marginHM.top - marginHM.bottom;

    const svgHM = d3.select("#graph-heatmap-sante").html("")
        .append("svg")
        .attr("viewBox", `0 0 ${widthHM + marginHM.left + marginHM.right} ${heightHM + marginHM.top + marginHM.bottom}`)
        .append("g")
        .attr("transform", `translate(${marginHM.left},${marginHM.top})`);

    const xHM = d3.scaleBand().domain(annees).range([0,widthHM]).padding(0.05);
    const yHM = d3.scaleBand().domain(categories).range([0,heightHM]).padding(0.05);
    const colorHM = d3.scaleSequential(d3.interpolateYlOrRd).domain([0,d3.max(data,d=>d.sante_limitee)]);
    ...
    svgHM.selectAll("rect")
        ...
        .on("mouseover", (event,d)=>{
            ...
            tooltip.html(`<strong>${d.csp_clean} (${d.annee})</strong><br/>
    ...
    svgHM.append("g").attr("transform",`translate(0,${heightHM})`).call(d3.axisBottom(xHM));
    // Axe Y Heatmap avec labels multi-lignes
    svgHM.append("g")
        .call(d3.axisLeft(yHM))
        .selectAll(".tick text")
        .each(function(d) {
            const [line1, line2] = splitText(d, 15); // 15 caractères max par ligne
            const text = d3.select(this);
            text.text(""); // vide le texte original
